  ...
  return (
    <div
      className={`relative w-1/3 ${
        group.place === 1 ? "h-5/6" : group.place === 2 ? "h-2/3" : "h-1/2"
      } bg-white bg-opacity-60 rounded-3xl flex flex-col items-center border-white border-2`}
    >
      <div
        className={`absolute ${
          group.place === 1 ? "-top-16 md:-top-28" : "-top-6 md:-top-12"
        } flex animate-bounce`}
      >
        <img src="/star.png" alt="Star" className="w-6 h-6 md:w-14 md:h-14" />
        {group.place === 1 && (
          <img
            src="/star.png"
            alt="Star"
